    ...
    if(have_aux[j]){
      ...
      if(!plugin.marginal[j]){
        for(r in 1:Rlevels[j]){
          ...
          # print("aux_quantile")
          # print(aux_quantile)
          # print("aux_quantile[aux_bin[ir_obs]-1]")
          # print(aux_quantile[aux_bin[ir_obs]-1])
          #
          # print("aux_quantile[aux_bin[ir_obs]]")
          # print(aux_quantile[aux_bin[ir_obs]])
          # print("j:")
          # print(j)
          #
          ...
          # print(ir_obs)
          #
          # # print("max(Zj[R[, j] == r - 1], na.rm = TRUE)")
          # # print(suppresswarnings(max(Zj[R[, j] == r -1], na.rm = TRUE)))
          #
          # print("aux_quantile[aux_bin[ir_obs]-1]")
          # print(aux_quantile[aux_bin[ir_obs]-1])
          #
          ...
          # if(length(lb) == 0 | length(ub) == 0 | any(is.na(lb)) | any(is.na(ub))){

            Zj[ir_obs] = Z_pastj[ir_obs]
          }
          # else if( any(is.na(lb)) | any(is.na(ub)) | any(lb>ub)){
          else if( any(lb>ub)){
